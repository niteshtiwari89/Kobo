...
import { createForm, getAllLibraryForms, updateForm, uploadImage, deleteImage } from "../api";
...
const CreateForm = () => {
  ...
  const [formTitle, setFormTitle] = useState(
    project ? project.title : template ? template.name : ""
  );
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [formDescription, setFormDescription] = useState(
    project ? project.description : template ? template.description : ""
  );
  ...
  const handleSaveForm = async () => {
    ...
    try {
      if (project?._id) {
        const response = await updateForm(project._id, formData); // Call updateForm API
  ...
  const addSection = () => {
    setSections([
      ...sections,
      {
        ...
        image: null, // Add image support for new sections
        questions: [
          {
            id: Date.now(),
...
//                     placeholder="Enter Question"
//                     value={question.text}
//                     onChange={(e) => updateQuestionOrOption(section.id, question.id, null, e.target.value, 'question')}
//                     className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200"
//                   />
//                   <div className="flex gap-2">
//                     <div className="relative">
...
//                           placeholder="Enter option"
//                           value={option.text}
//                           onChange={(e) => updateQuestionOrOption(section.id, question.id, option.id, e.target.value, 'option')}
//                           className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200"
//                         />
//                         <button
...
// //   const [formDescription, setFormDescription] = useState(project ? project.description : '');
// //   const [sections, setSections] = useState(project ? project.sections : [
// //     {
// //       id: Date.now(),
// //       title: 'Untitled Section',
// //       visibilityCondition: null, // No condition initially
...
// //   // Update question or option text
// //   const updateQuestionOrOption = (sectionId, questionId, optionId, newText, type = 'question') => {
// //     setSections(sections.map(section => {
// //       if (section.id === sectionId) {
// //         return {
// //           ...section,
...
// //   const handleVisibilityConditionChange = (sectionId, condition) => {
// //     setSections(sections.map(section => {
// //       if (section.id === sectionId) {
// //         // Update the visibility condition and evaluate visibility
// //         let isVisible = true;

// //         // Logic to set visibility based on the condition
// //         if (condition === 'checkbox') {
// //           // For this example, toggle visibility based on some checkbox condition
// //           isVisible = section.visibilityCondition === 'checkbox' && section.isVisible;
// //         }

// //         return { ...section, visibilityCondition: condition, isVisible };
// //       }
// //       return section;
// //     }));
// //   };
...
// //     setLoading(true);
// //     try {
// //       const response = await createForm(formData); // Call the createForm API
// //       console.log('Form created successfully:', response);
// //       alert('Form created successfully!');
// //     } catch (error) {
// //       console.error('Error creating form:', error);
...
// //                 />
// //                 <button
// //                   onClick={() => handleVisibilityConditionChange(section.id, 'checkbox')}
